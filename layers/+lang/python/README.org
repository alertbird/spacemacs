...
*** Microsoft python language server
Microsoft python language server will be automatically installed in the =~/.emacs.d/mspyls=
directory by the =ls-python-ms= package, when you use the python mode with the python lsp
server backend =mspyls= for the first time.
